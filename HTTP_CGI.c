...
void cgi_process_var (U8 *qs) {
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (str_scomp (var, "ModoManual=ON") == __TRUE) {
        statechart=MANUAL;
				estadouart=MODO_SELECCIONADO;
				buffer[0]='M';
				LCD_Clear(Black);
      }
      else if (str_scomp (var, "ModoAutomatico=ON") == __TRUE) {
        statechart=AUTOMATICO;
				estadouart=MODO_SELECCIONADO;
				buffer[0]='A';
				LCD_Clear(Black);
      }
			else if (str_scomp (var, "ModoConfiguracion=ON") == __TRUE) {
        statechart=CONFIGURACION;
				estadouart=MODO_SELECCIONADO;
				buffer[0]='C';
				LCD_Clear(Black);
      }
      else if (str_scomp (var, "ApagarAlarma=ON") == __TRUE) {
         AlarmaActivada=0;

      }
      else if (str_scomp (var, "EncenderAlarma=ON") == __TRUE) {
         AlarmaActivada=1;
      }
   }
  }while (qs);					
    ...
    case 'a' :
      switch (env[2]) {
        case '3':
          ...
          len = sprintf((char *)buf,(const char *)&env[4],AlarmaActivada ? "red" : "green");
          break;
        case '4':
          /* Write local Net mask. */
          len = sprintf((char *)buf,(const char *)&env[4],AlarmaActivada ? "green" : "red");
          break;
           break;
				case '5':
          /* Write the local IP address. The format string is included */
          /* in environment string of the script line.                 */
          len = sprintf((char *)buf,(const char *)&env[4],AnguloMotor);
          break;
        case '6':
          /* Write local Net mask. */
          ...
          /* Write the local IP address. The format string is included */
          /* in environment string of the script line.                 */
          len = sprintf((char *)buf,(const char *)&env[4],DistanciaMedida);
          break;
        case '8':
          /* Write local Net mask. */
          len = sprintf((char *)buf,(const char *)&env[4],brilloTFT);
          break;
        default:
           break;
      }
